...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    for stats in statistics:
        compound_stats = formula(info,stats)
        current_player = (stats[info['playerid']],compound_stats)
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    table = read_csv_as_list_dict(filename,',','"')

    for player in top_ids_and_stats:
        current_id = player[0]
        for stat in table:
            if stat[info['playerid']]==current_id:
                first_name = stat[info['firstname']]
                last_name = stat[info['lastname']]
                score = player[1]
                score = "%.3f" % score
                final_string = score+" --- "+first_name+" "+last_name
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    filter_year = filter_by_year(table,year,info["yearid"])

    top_ids = top_player_ids(info,filter_year,formula,numplayers)

    result = lookup_player_names(info,top_ids)
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    for stat in statistics:
        ...
        temp={}
        current_id = stat[playerid]
        temp[playerid]=current_id
        for names in fields:
            add=0
            for count in statistics:
                if current_id == count[playerid]:
                    add=add+int(count[names])
            temp[names]=add
            add=0
        result[current_id]=temp    
