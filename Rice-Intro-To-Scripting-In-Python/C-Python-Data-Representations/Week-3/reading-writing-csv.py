...
def lookup_player_names(info, top_ids_and_stats):
    ...
    with open(info['masterfile'],newline='') as csvfile:
      ...
      for player in reader:
        player_names[player[info['playerid']]] = (player[info['firstname']], player[info['lastname']])

      for top_id in top_ids_and_stats:
        player_info.append('{:0.3f} --- {} {}'.format(top_id[1], player_names[top_id[0]][0],player_names[top_id[0]][1]))
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    with open(info['battingfile'], newline = '') as csvfile:
      ...
      return lookup_player_names(info, top_player_ids(info, stats, formula, numplayers))#print(compute_top_stats_year({'masterfile': 'master3.csv', 
